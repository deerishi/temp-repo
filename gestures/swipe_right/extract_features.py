...
def extract_features():
	...
	line=f.readline().split('\n')[0]
	fz=open(line,'r')
	vector_file=open(r'train_swipe_right_10_dim_features','w')
	#label_file=open(r'label_minus_1_swipe_right.txt','w')
	...
	print ' the final feature vector array is ',final_feature_array,' and length of data ',len(final_feature_array[0]),' and dimensions are ',len(final_feature_array)
	np.save(vector_file,final_feature_array)
	vector_file.close();
	#label_array=np.asarray(labels)
	#print 'the label array is ',label_array
	#np.save(label_file,label_array)
	#print 'the of label size is ',len(label_array),' and of data ',len(final_feature_array[0]),' and dimensions are ',len(final_feature_array)
