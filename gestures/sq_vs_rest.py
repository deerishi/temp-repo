...
def main():
	...
	len_right1=len(data_sq[0])
	
	data_left_p1=np.load(fright)
	data_left_p2=np.load(fleft)
	length_r_by2=int(len(data_sq[0])/2) # this is the length of the training dataset with label 1
	data_left_1=data_left_p1[:,0:length_r_by2]
	if len_right1%2==1:
		data_left_2=data_left_p2[:,0:length_r_by2+1]
	else:
		data_left_2=data_left_p2[:,0:length_r_by2]
	data_left=np.hstack((data_left_p1,data_left_p2))
	...
	normalization_file=open('normalization_parameters_for_sq_vs_rest.txt','w')
	...
	file_name="square_vs_rest/accuracy="+ str(evaluator2.get_accuracy()) + " liblinear_square_vs_rest_svm_classifier_with_C_10_and_normalized.h5"
